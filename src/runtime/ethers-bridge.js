//
// plugin/ethers-bridge.js
//
// Replaces the network-touching surface of `hre.ethers` with a
// TronWeb-backed equivalent so unmodified upstream OZ tests can run
// against the local TRE container.
//
// What we replace:
//
//   hre.ethers.deployContract(name, args?, signer?)
//     → load artifact by name from artifacts-tron/, deploy via TronWeb,
//       return a Contract-shaped Proxy (same shape as
//       test/helpers/ethers-facade returns) so the test sees
//       `factory.target`, `await factory.method(args)`, etc.
//
//   hre.ethers.getContractFactory(name, signer?)
//     → return a Factory-shaped object exposing `.bytecode`,
//       `.interface`, `.attach(addr)`, `.deploy(...args)`. Read-only
//       methods on the factory don't touch the network — they only
//       read the cached artifact JSON, so `factory.bytecode` and
//       `factory.interface.encodeDeploy([...])` work without any
//       round-trip.
//
//   hre.ethers.getSigners()
//     → list of pseudo-signer objects with `.address` (0x-prefixed,
//       20-byte body of the TRE deployer's TRON address) and
//       `getAddress()`. The TRE container only exposes one pre-funded
//       account by default, so `getSigners()` returns the same key
//       wrapped as multiple "signers" — they all sign as the same
//       account on chain. Tests that need *separate* sender
//       identities should fail under this; that failure is a real
//       TVM-vs-EDR delta worth surfacing.
//
// What we LEAVE ALONE:
//   * hre.ethers.utils helpers (id, keccak256, AbiCoder, getCreate2Address, ...)
//   * Pure-arithmetic ethers exports (Typed, MaxUint256, ZeroAddress, ...)
//
// hardhat-ethers's own `extendEnvironment` runs first (it's required
// before `./plugin` in hardhat.config.cjs); we run last and overwrite
// the parts we want to redirect.
//
...
// Inject ethers into cheatcodes so its lazy `getCreateAddress` call for EVM
// CREATE prediction can run without forcing a hard `require('ethers')` at the
// cheatcodes module's top level (it has to stay loadable without ethers for
// the Sui/Aptos sibling spikes that don't ship it).
...
// stubProvider is shared across all signers/contracts in the spike.
// The methods below are filled in at hook-time once `hre` is
// available (we need TronWeb to actually query chain state). Until
// then they return sane defaults for tests that probe them at load
// time.
const stubProvider = {
  _hre: null,
  _tw() {
  ...
  async getBalance(address) {
    const tw = this._tw();
    if (!tw) return 0n;
    // EVM-pred → TVM-actual rewrite so `provider.getBalance(clone.target)` for
    // a Clones-style proxy queries the address TVM actually deployed to. See
    // cheatcodes.js _evmToTvm.
    const remapped = lookupTvmActualBase58(address);
    const addr = remapped || signersMod.toBase58(address);
  ...
  async getBlock(blockHashOrNumber) {
    const tw = this._tw();
    if (!tw) return null;
    const tag = blockHashOrNumber === 'latest' || blockHashOrNumber == null
      ? undefined
      : blockHashOrNumber;
...
function splitOverrides(args, expectedInputCount = null) {
  ...
  if (last && typeof last === 'object') {
    ...
    if (
      ...
      args.length === expectedInputCount + 1 &&
      !('_typedSymbol' in last)
    ) {
...
function normalizeArg(value, input, nested = false) {
  ...
  if (input.type === 'address') {
    // EVM-pred → TVM-actual rewrite. Extract the address string first, THEN
    // look up the mapping. The value may be a contract proxy ({target: '0x…'})
    // or a signer ({address: 'T…'}) — both routes flow through
    // asAddressString. See _evmToTvm in cheatcodes.js.
...
function translateReceipt(txId, info, iface = null) {
  ...
  const energyUsed = info && info.receipt && Number.isFinite(info.receipt.energy_usage_total)
  ...
  return {
    ...
    // replaced by plugin/chai-matchers-tvm.js. blockHash is exposed
    // for tests that read it directly. java-tron's `blockHash` is
    // not on the unconfirmed-info shape, so this is best-effort.
    blockHash: info && info.blockHash ? (info.blockHash.startsWith('0x') ? info.blockHash : '0x' + info.blockHash) : undefined,
...
async function staticCallWithRevertData(fnAbi, tronContract, tronWeb, normalized) {
  ...
  const calldata = iface.encodeFunctionData(ifaceFragment, normalized.map((v, i) => coerceForEthers(v, fnAbi.inputs[i])));
  ...
  // EVM-pred → TVM-actual rewrite for the contract address being called. The
  // ethers contract proxy binds to the address staticCall returned (which the
  // simulator computed using EVM CREATE rules); subsequent view calls on that
  // proxy target the EVM-predicted address. Rewrite to the TVM-actual one if
  // we've seen the CREATE happen since. See _evmToTvm in cheatcodes.js.
  ...
  if (!reverted) {
    ...
    // If this function returned an address-typed value (e.g. `clone()` or
    // `cloneDeterministic()` returning the new contract address), queue it as
    // a "pending simulator-returned address" keyed by the caller contract.
    // The next CREATE in an actual broadcast from this caller pairs with it
    // via registerCreateMappingsFromReceipt (sequence-based), giving us a
    // simulator-addr → broadcast-addr mapping without having to reverse-
    // engineer java-tron's CREATE derivation. See _evmToTvm comment.
    try {
      if (fnAbi.outputs.length === 1 && fnAbi.outputs[0].type === 'address') {
        const addr = finalDecoded && typeof finalDecoded === 'string' ? finalDecoded :
          (Array.isArray(finalDecoded) ? finalDecoded[0] : undefined);
  ...
  // TVM precompile-mismatch recovery. Java-tron only supports the legacy
  // three EVM precompiles (0x01 ecRecover, 0x02 SHA256, 0x03 RIPEMD160) at
  // their canonical addresses. A Solidity `staticcall(0x04..0x0a, ...)` —
  // which EVM treats as a call to a missing-code address (success=true with
  // empty returndata under post-Constantinople rules) — aborts the whole
  // simulation on TVM with an empty constant_result and a TVM-level error
  // code. OZ's `SignatureChecker.isValidERC1271SignatureNow` and similar
  // patterns rely on the EVM behavior: the inner staticcall returns empty,
  // the length check fails, and the outer function returns false. To match
  // that, when we observe a reverted simulation with no constant_result data
  // AND we can attribute the failure to a TVM-unsupported precompile address
  // appearing in the call arguments, synthesize the zero-decoded return.
  if (raw === '0x' && hasTvmUnsupportedPrecompileArg(normalized, fnAbi.inputs)) {
    try {
      ...
      // (uint/bool/address/bytes32 → 0) and dynamic decoders (offset → 0,
      // length → 0, data empty).
      ...
      // Outputs include a shape we can't safely fabricate. Fall through to
      // the original error so the symptom surfaces instead of bad data.
    }
  }
  ...
  // selector for the function's output type, surface that selector. See
  // the matching synthesis in makeMethod.invoke for the write path.
  if (raw === '0x') {
    const synthSelector = (fnAbi.outputs.length > 0 && fnAbi.outputs[0].type === 'address')
      ? '0xb06ebf3d'  // FailedDeployment()
      : '0xd6bda275'; // FailedCall()
    try {
      const codeInfo = await tronWeb.trx.getContract(tronContract.address);
...
// Returns true iff any address-typed arg in `args` (TronWeb-normalized, so
// addresses are base58 `T...` strings) corresponds to an EVM precompile in
// the range [0x04..0x0a] that java-tron does not ship. Used by the recovery
// path in `staticCallWithRevertData` to differentiate "real revert with
// missing data" from "TVM aborted on inner staticcall to missing precompile".
...
// deployer.
async function sendImpersonatedCall(fnAbi, contractTronBase58, fromTronBase58, normalized, deployerTronWeb, overrides = {}) {
  const { fragment: ifaceFragment, iface } = getFragmentAndIface(fnAbi);
  const sig = canonicalSig(fnAbi);
  const calldata = iface.encodeFunctionData(ifaceFragment, normalized.map((v, i) => coerceForEthers(v, fnAbi.inputs[i])));
  const parameter = calldata.slice(10);

  // msg.value override. 1 wei == 1 sun pass-through (see
  // cheatcodes.js top-of-file unit-model comment).
  let callValueSun = 0;
  ...
  // the BigInt-safe JSON serializer; the Java FullNode parses
  // `call_value` as a Long.
  const { jsonStringifyWithBigInts } = require('./cheatcodes');
  const triggerResp = await fetch(base + '/wallet/triggersmartcontract', {
    ...
    body: jsonStringifyWithBigInts({
      ...
      function_selector: sig,
      parameter,
      // Default to TRE's 1_000_000_000 sun (≈10M energy @ 100 sun/energy)
      // ceiling. Honor `overrides.feeLimit` (already mapped from
      // `overrides.gasLimit` by makeMethod.invoke) when supplied so OOG
      // probes actually constrain the budget.
  ...
  if (!triggerJson.transaction || !triggerJson.result || triggerJson.result.code) {
    throw new Error(
      `triggersmartcontract failed (impersonated from ${fromTronBase58}): ` +
  ...
  // the whole point. Passing the `multisig` flag (4th arg) bypasses
  // the owner-key check while keeping the signTransaction primitive
  // intact. The signature itself ECRecovers to the deployer; the
  // patched TransactionCapsule.validateSignature accepts that
  // because owner is on TreImpersonationRegistry.
  // privateKey=undefined falls back to tronWeb.defaultPrivateKey
  // (set when buildTronWeb constructs the deployer's instance). The
  // signature ECRecovers to the deployer; the patched
  // TransactionCapsule.validateSignature accepts that because owner
  // is on TreImpersonationRegistry.
  const signed = await deployerTronWeb.trx.sign(
    triggerJson.transaction,
    undefined,
    undefined,
    /* multisig */ true,
  );
...
// Build the right error to throw on a reverted state-changing call.
// TVM doesn't propagate inner-call revert data through the outer
// tx's `contractResult` field — for calls like
// `AccessManager.consumeScheduledOp(...)` (which staticcalls
// `IAccessManagerCallback.isConsumingScheduledOp()` and then reverts
// with a custom error), the receipt's `contractResult[0]` is empty
// and `buildRevertError` returns `.data='0x'`. chai-matchers' then
// reports "reverted without a reason" instead of decoding the custom
// error.
//
// Recover the revert data by re-running the call via
// `staticCallWithRevertData` — `/wallet/triggerconstantcontract`
// always returns `constant_result` with the encoded revert payload.
// Only invoked when the receipt is REVERT and data is empty.
async function buildRevertErrorWithSim(txId, info, fnAbi, tronContract, tronWeb, normalized) {
  ...
  // Sim succeeded (returned a value) but broadcast reverted with no data. The
  // EVM-style triggerconstantcontract simulator and the TVM-style broadcast
  // use different CREATE/CREATE2 address derivations, so a "deploy twice
  // with the same salt" check that OZ wraps as
  ...
  // broadcast-revert with a successful simulator outcome is the canonical
  // signature for this case.
  if (simErr === undefined && fnAbi.outputs.length > 0 && fnAbi.outputs[0].type === 'address') {
    try {
      const code = await tronWeb.trx.getContract(tronContract.address)
        .then(info => info && info.bytecode ? '0x' + info.bytecode.toLowerCase() : '0x');
...
function makeMethod(fnAbi, tronContract, tronWeb, hre) {
  ...
  async function invoke(...rawArgs) {
    ...
    const _gasLimitSun = overrides.gasLimit != null
    ...
    if (tronWeb._impersonatedFrom) {
      ...
      let impCallValueSun = 0;
      if (overrides.value != null) {
        const { valueToCallValue } = require('./cheatcodes');
        impCallValueSun = valueToCallValue(overrides.value);
      }
    ...
    // when nested addresses are hex-formatted, throwing "cannot encode
    // object for signature with missing names". Symptom in OZ tests:
    // `ERC2771Forwarder.execute(ForwardRequestData)`, MultiSig batch
    // calls (`tuple[]` args), `Multicall.aggregate(Call[])`.
    //
    // Workaround: ethers-encode the calldata ourselves and broadcast
    // through the same raw HTTP path the impersonation case uses.
    // `multisig: false` keeps TronWeb's owner-key check active — the
    // current signer's TronWeb HAS the matching private key, so it
    // signs normally.
    const hasTupleInput = fnAbi.inputs.some(
      i => i && (i.type === 'tuple' || /^tuple\[/.test(i.type)),
    );
    if (hasTupleInput) {
      const { sendCallTx } = require('./cheatcodes');
      const { fragment: ifaceFragment, iface } = getFragmentAndIface(fnAbi);
    ...
    try {
      ...
      if (/Not enough energy|OUT_OF_ENERGY|REVERT/.test(msg)) {
        ...
        if (simNoData) {
          ...
          } catch (e) { if (e && e.data) throw e; /* else swallow code-fetch errors */ }
        }
        // Re-throw whatever sim got us, preferring sim's err over the raw
        // TronWeb error since the latter is often just "energy not enough".
...
//
// The fix: `signer.predictAddress(name, args, opts)` (added in
// plugin/signers.js) builds + signs a CreateSmartContract tx via
...
function prebuildCacheKey(deployerBase58, name, args) {
  ...
  // sentinel — same approach as plugin/cheatcodes.js's
  // `jsonStringifyWithBigInts`. We don't need to send this to TVM, just
  // a stable string for Map keying, so the simpler form suffices.
  const stringify = v => {
...
async function deployViaFacade(hre, name, args = [], signer = null) {
  ...
  // 1 wei == 1 sun pass-through (plugin/cheatcodes.js top-of-file). For
...
// Builds (or returns cached) array of distinct signers from
// plugin/signers.js. Each signer carries its own TronWeb instance
// bound to its private key, so `contract.connect(signer).method(...)`
// sends a transaction signed by that signer's key on chain.
...
// `_impersonatedFrom` so makeMethod / staticCallWithRevertData route
// writes through the forging path. Compatible with
// `contract.connect(signer)` and `account.js::impersonate(...)`.
//
// signMessage / signTypedData are deliberately omitted — impersonation
// is for msg.sender-driven flows, not off-chain signature flows.
function makeImpersonatingSigner(hre, address) {
  ...
  const deployerTronWeb = hre.tre.makeTronWeb().tronWeb;
  // Prototype-clone the deployer's TronWeb so all methods + state
  ...
  const signer = {
    ...
    // Impersonated `signer.sendTransaction({to, data, value})` — used
    // by upstream OZ tests that send arbitrary calldata "as" the
    // impersonated address (e.g. AccessManager.predicate.js's
    // scheduled-op direct-call cases). Same triggersmartcontract path
    // as makeMethod's impersonated write branch, with multisig=true
    // so TronWeb's owner-key check is bypassed. Returns a
    // TransactionResponse-shaped object.
    async sendTransaction({ to, data, value } = {}) {
      const toBase58 = signersMod.toBase58(to);
      // Routing matches signers.js: any `data` field (including '0x')
      // → TriggerSmartContract; `data === undefined` → plain TRX
      // transfer. `multisig: true` bypasses TronWeb's owner-key
      // check; the patched fork accepts forged owner via the
      ...
      // Plain TransferContract → skip waitForReceipt (no receipt
      // field, would time out). See signers.js for the same gate.
...
// loadFixture wired to tre_snapshot / tre_revert.
//
// Mirrors @nomicfoundation/hardhat-network-helpers' implementation:
//   * Array (not Map) of { restorer, fixture, data }, looked up by
//     fixture-function reference.
//   * On cache hit: restorer.restore() (= tre_revert + fresh
//     tre_snapshot), then any snapshot taken AFTER this one is
//     discarded (matches Hardhat's "older snapshot invalidates
//     newer" semantics, which is also what our Java fork enforces
//     via the LinkedHashMap walk in TreJsonRpcImpl.revert).
//   * Anonymous fixtures (fn.name === "") are rejected, same
//     reason as upstream — every cache lookup is by reference, and
//     re-defining an anonymous arrow per test gives a fresh
//     reference each time, silently busting the cache and going
//     back to per-test redeploy.
//
// Why this beats per-test redeploy: tre_revert is a memory
// state-dump restore (~20-50ms for the OZ test surface), whereas
// a TVM deploy is several seconds of bytecode publish + receipt
// poll. For a 100-test file with one shared fixture, that's a
// minute-vs-an-hour difference.
//
// Fallback: on stock tronbox/tre (no `tre_snapshot`) we run the
// fixture every call — correctness over speed.
...
async function _supportsSnapshot(tronWeb) {
  ...
  _snapshotSupported = !!(v && v.result && /oz-spike/.test(v.result));
...
async function loadFixture(fn) {
  ...
  // includes any AccountStore entries created by `tre_setAccount
  // Balance`). Symptom: "Contract validate error : account [T...]
  // does not exist" when a test routes a tx through one of the
  // derived signers. Idempotent — buildSigners caches the array.
  ...
  if (snapshot === undefined) {
    ...
    // DEFAULT_BALANCE_SUN before fn() runs and the snapshot is taken.
    // Without this, a previous test file's spending leaves the
    // deployer's balance well below what a Governor / metatx fixture
    // expects, and the fixture errors with "balance is not sufficient"
    // when it sends e.g. `parseEther("1")` to the contract under test.
    // tre_setAccountBalance is idempotent (direct state mutation, not
    // a transfer), so re-funding has no side effects on chain history.
    await signersMod.refundSigners(hre);
  }
  if (snapshot !== undefined) {
    // Capture the snapshot id BEFORE restore() — restore() reverts to
    // it and then takes a fresh snapshot, overwriting
    // `restorer.snapshotId` with the NEW (higher) id. Newer snapshots
    // in the array were taken between this fixture's last restore and
    // now; the server-side revert() walked the LinkedHashMap newest-
    // first and dropped everything from `oldId` onwards, so every
    // _fixtureSnapshots entry with id > oldId now references a server
    // snapshot that no longer exists. Without this eviction, the next
    // loadFixture for one of those evicted fixtures hits
    // `tre_revert(unknown_id)` and the whole describe-block "before
    // each" hook errors out (see test/utils/Arrays.test.js's nested
    // fixtures pattern).
    const oldId = Number(snapshot.restorer.snapshotId);
    await snapshot.restorer.restore();
    const evicted = _fixtureSnapshots.filter(
      s => s !== snapshot && Number(s.restorer.snapshotId) > oldId,
    );
    _fixtureSnapshots = _fixtureSnapshots.filter(
      s => s === snapshot || Number(s.restorer.snapshotId) <= oldId,
    );
...
// Tests imported from upstream OZ do
//   const { loadFixture } = require('@nomicfoundation/hardhat-network-helpers')
// at module-load time. The original implementation calls
// `checkIfDevelopmentNetwork(hre)` which throws OnlyHardhatNetworkError
// on any non-Hardhat network. We can't modify the tests, but we can
// redirect what the require resolves to.
//
// The package's `index.js` re-exports `loadFixture` through generated
// `Object.defineProperty(..., { get: () => mod.loadFixture })` proxies,
// so mutating the index export directly silently fails (no setter on
// the getter). Instead we mutate the underlying
// `dist/src/loadFixture.js` module's `loadFixture` value — the
// getter reads the live value, so subsequent `require('...network-
// helpers')` calls (including the destructure in the test file)
// resolve to our function.
// Patch the dist/src/* modules of `@nomicfoundation/hardhat-network-
// helpers` so unmodified upstream OZ tests resolve to TVM-aware
// versions. The package's `index.js` re-exports each function via
// `Object.defineProperty(..., { get: () => mod.<name> })` getters with
// no setter — so we mutate the BACKING MODULE'S export, which the
// getter reads on every access. Mutating index.* directly silently
// fails.
...
function patchHardhatProvider(hre) {
  ...
  const time = require('./time');
  const { rpcCall, mine: tre_mine } = require('./cheatcodes');
  ...
  async function handleEthRpc(method, params) {
    if (method === 'eth_estimateGas') {
      // Best-effort gas estimate via triggerconstantcontract. Tests use this
      // to compute "less than estimate" gas limits for OOG forcing
      ...
      const fromBase58 = txArg.from
        ? signersMod.toBase58(txArg.from)
...
// Install a root-level mocha `beforeEach` that bumps the per-test timeout from
// the default 2000ms to 30 minutes. TVM runs MUCH slower than the EVM-on-
// JS-VM mocha defaults assume: per-block production is ~3s, multi-tx Governor
// proposal/queue/execute flows easily span dozens of blocks, and the long
// parallel-run mocks add chain-state overhead that pushes individual `it()`
// blocks past the 600s mocha default we already see hit on multiple tests
// (test/governance/extensions/GovernorSequentialProposalId.test.js,
// test/governance/extensions/GovernorSuperQuorum.test.js, etc).
//
// Mocha's `beforeEach` global only exists once mocha is loaded by the
// `hardhat test` task. We poll briefly for it on `setImmediate` so we register
// before any test describes run.
...
function patchNetworkHelpers(hre) {
  ...
  // runtime whitelist on TransactionCapsule.validateSignature so any
  // tx whose owner_address is whitelisted skips ECRecover. The bridge
  // then uses the impersonating-signer path (makeImpersonatingSigner
  // + sendImpersonatedCall) to broadcast txs with forged
  // owner_address, letting OZ tests' `msg.sender` checks succeed for
  // contract-as-caller flows (AccessManaged, Bridge*, RelayedCall, …).
  //
  // On an UNPATCHED fork the RPC is method-not-found → callOrFalse
  // returns `{supported: false}`. We still resolve the helper (so
  // beforeEach doesn't fail-fast and trigger the fixture-rerun storm),
  // but skip the local-registry add. Subsequent impersonated writes
  // will fail at broadcast with a clear "rebuild the fork" error.
  tryPatch(
  ...
  tryPatch(
    '@nomicfoundation/hardhat-network-helpers/dist/src/helpers/mine',
    'mine',
    async (blocks = 1, _opts = {}) => time.mine(tw(), blocks),
  );
  tryPatch('@nomicfoundation/hardhat-network-helpers/dist/src/helpers/mineUpTo', 'mineUpTo', async target =>
    time.mineUpTo(tw(), target),
  );

  // --- time.latest / time.latestBlock (read primitives, backed by getCurrentBlock) ---
  ...
  // --- time.advanceBlock / time.advanceBlockTo (block-number primitives) ---
  ...
  // Real-time waits backed by tre_blockTime + auto-mine. Cost ~1:1
  // wall:chain. Suitable for OZ tests with shortened parameters (e.g.
  // votingDelay=1, votingPeriod=2 for timestamp-mode governors); not
  // viable for unmodified tests that expect 1-week voting periods.
  tryPatch(
    '@nomicfoundation/hardhat-network-helpers/dist/src/helpers/time/increase',
    'increase',
    async seconds => time.increase(tw(), seconds),
  );
  tryPatch(
    '@nomicfoundation/hardhat-network-helpers/dist/src/helpers/time/increaseTo',
    'increaseTo',
    async target => time.increaseTo(tw(), target),
  );
  tryPatch(
  ...
  // --- setCode / getCode / setStorageAt / getStorageAt / setBalance ---
  // Wrap the corresponding `tre_*` cheatcodes. Many upstream tests
  // (Packing, StorageSlot, Multicall, UUPS, Beacon, ...) use
  // `forceDeployCode(name, addr)` from test/helpers/deploy.js which
  // calls `setCode(addr, deployedBytecode)`. Without this redirect,
  // those tests fail at module load.
  ...
  tryPatch(
    ...
    async (address, slot) => {
      const addr = signersMod.toBase58(address);
      const slotHex = typeof slot === 'bigint' ? ethersV6.toBeHex(slot, 32) : slot;
      // TronWeb doesn't expose per-slot storage directly; we use the
      // debug_storageRangeAt RPC which TRE inherits from java-tron.
  ...
  tryPatch('@nomicfoundation/hardhat-network-helpers/dist/src/helpers/setBalance', 'setBalance', async (address, balance) => {
    const addr = signersMod.toBase58(address);
    // 1 wei == 1 sun pass-through (see plugin/cheatcodes.js
    // top-of-file unit-model comment). java-tron's TreUtil.decodeLong
    // accepts up to Long.MAX_VALUE (~9.22e18 sun) — tests passing
    // parseEther("10") or above will overflow. Cap to Long.MAX_VALUE
    // so the test sees a useful error rather than a generic
...
extendEnvironment(hre => {
  ...
  // calls that our java-tron FullNode doesn't serve; our replacements
  // compute balance changes from receipt logs + internal_transactions.
  // chai's `Assertion.addMethod` REPLACES prior registrations so
  // ordering vs hardhat-chai-matchers doesn't matter — last wins.
  try {
    require('./chai-matchers-tvm').register();
  } catch (e) {
    // chai not yet installed in this resolution? Test-time `require
    // ('chai')` will surface the error if so. Don't fail boot here.
  }
  if (!hre.ethers) return;

  // TVM-aware CREATE2 address prediction. EVM's `keccak256(0xff || deployer
  // || salt || keccak256(initcode))[12:]` becomes `keccak256(0x41 || deployer
  // || salt || keccak256(initcode))[12:]` on TVM — java-tron uses 0x41 as the
  // CREATE2 prefix (the same prefix used on every TVM address). OZ's
  // Solidity-side `Create2.computeAddress` + `Clones.predictDeterministicAddress`
  // are already patched in the spike's contracts/utils/Create2.sol and
  // contracts/proxy/Clones.sol; this override aligns the off-chain
  // `ethers.getCreate2Address(deployer, salt, initCodeHash)` predictions
  // the unmodified upstream OZ tests rely on (test/proxy/Clones.test.js:132,
  // test/utils/Create2.test.js:41/55/66/80/100, …). Without this, on-chain
  // and off-chain predictions disagree by the prefix byte and the address-
  // already-used / deterministic-prediction tests fail with mismatched
  // addresses on every fixture.
  hre.ethers.getCreate2Address = (deployer, salt, initCodeHash) => {
    const deployerHex = ethersV6.getAddress(deployer); // checksums + 0x-hex (20 bytes)
    const saltHex = ethersV6.zeroPadValue(ethersV6.isBytesLike(salt) ? salt : ethersV6.toBeHex(salt), 32);
    const initHex = ethersV6.zeroPadValue(initCodeHash, 32);
    const packed = ethersV6.concat(['0x41', deployerHex, saltHex, initHex]);
    return ethersV6.getAddress(ethersV6.dataSlice(ethersV6.keccak256(packed), 12));
  };

  // Replace the network-touching methods. Pure helpers (utils,
  // AbiCoder, id, keccak256, getCreateAddress, getCreate2Address, etc)
  // stay as ethers ships them — they emit EVM-formula predictions, and
  // any divergence from TVM's actual deploy address (which uses a 0x41
  // prefix for CREATE2 and a tx-hash + sender derivation for CREATE)
  // is left visible to tests instead of silently corrected.
  // `args` is positional in upstream OZ tests (`deployContract(name, [a,b])`)
  // and sometimes followed by a signer (`deployContract(name, [a,b], signer)`).
  // The 3rd arg can also be a plain `{value, gasLimit, ...}` overrides
  // object (proxy tests pass `{value}` here to fund construction). Detect
  // and route into the args list as a trailing overrides element so
  ...
  // Singular `getSigner(addr)` — the entry point for
  // test/helpers/account.js::impersonate. Locally-impersonated
  // addresses (registered by patchNetworkHelpers' impersonateAccount
  // hook above) get a pseudo-signer whose write path forges
  // owner_address. An address that doesn't match any derived signer
  // is treated as a future-impersonation request (upstream OZ does
  // `this.caller = await getSigner(contract.target); await impersonate
  // (this.caller.address);` — calls getSigner BEFORE registering the
  // impersonation, so we can't check the registry eagerly). Return an
  // impersonating signer for any unknown address; the tx broadcast
  // path still needs the patched FullNode to whitelist `owner_address`
  // (via `impersonate(...)`), so callers that forget to register get
  // a clear "owner_address not impersonated" rejection from the chain
  // instead of a silent fallback to the deployer key (which used to
  // produce the wrong `msg.sender` and confusing empty-data reverts).
  ...
  // CREATE2 on TVM uses `keccak256(0x41 || sender || salt || initHash)[12:]`
  // — verified against on-chain deploys, and the in-tree CREATE2 libraries
  // (contracts/utils/Create2.sol, contracts/proxy/Clones.sol,
  // contracts/utils/RelayedCall.sol) have all been TVM-patched to hash
  // with the `0x41` prefix as well. This override is the matching half
  // for those contract-side patches: every test that compares an
  // off-chain prediction against `factory.$computeAddress(...)` /
  // `factory.predictDeterministicAddress(...)` would otherwise see
  // EVM-formula (0xff) on the JS side vs TVM-formula (0x41) in the
  // contract. Keeping the override consistent with the patched
  // contracts is the only way the predictions line up; without it the
  // CREATE2 test suites all fail with prefix-byte address mismatches.
  ...
  // chai-matchers' `reverted` matcher hits hre.ethers.provider.get
  // TransactionReceipt directly. Wire that through our cache.
  if (hre.ethers.provider) {
    ...
    // Route `ethers.provider.getBalance(addr)` / `getCode(addr)` through
    // the TronWeb-backed stubProvider so:
    //   1. We can apply EVM-pred → TVM-actual rewrites (Clones / CREATE)
    //   2. We don't depend on TRE's eth_getBalance (which mishandles
    //      non-`latest` block tags and a few address forms).
    // Tests that read `await ethers.provider.getBalance(this.proxy)` for a
    // Clones-style instance now see the correct TVM-side balance.
    hre.ethers.provider.getBalance = async (address) => stubProvider.getBalance(address);
    hre.ethers.provider.getCode = async (address) => stubProvider.getCode(address);
  }
});
...
module.exports = {
  ...
  makeSigners,
  loadFixture,
  // Internal — exported so signers.js can lazy-require it to build
  // ethers-shaped revert errors with `.data` for chai-matchers'
  // `revertedWithCustomError`. Lazy require sidesteps the
  // ethers-bridge ↔ signers module-load cycle.
  buildRevertError,
  // Address utilities. Surfaced on `hre.tre.*` from plugin/index.js so
  // tests can convert between TRON base58 and ethers checksummed hex
  // without importing from `plugin/ethers-bridge.js` directly.
  tronToHex,
  toTronBase58,
  // Internal — exported so plugin/signers.js's `signer.predictAddress`
  // can stash a pre-signed CreateSmartContract tx that the next
