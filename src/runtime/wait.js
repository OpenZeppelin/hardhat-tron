...
// plugin/wait.js
//
// Poll the Full Node's unconfirmed view for a tx receipt. The Solidity
// Node only returns data for solidified blocks, which never happens on
// a single-node regtest TRE — so we hit `getUnconfirmedTransactionInfo`
// first and only fall back to the solidified view as a hedge for public
// networks.
//

const { mine } = require('./cheatcodes');

// JSON parser that preserves integer precision above 2^53.
// TVM's transaction-info response embeds `callValueInfo[*].callValue`
// (and similar fields) as raw JSON integers up to Long.MAX_VALUE
...
async function waitForReceipt(tronWeb, txId, { timeout = 240_000 } = {}) {
  ...
  // tests that depend on precise block-count semantics (e.g.
  // ERC20Votes' "recent checkpoints" test expecting 6 mints = 6 blocks).
  //
  ...
  //
  // Poll cadence: yield via `setImmediate` between RPC fetches
  // instead of `setTimeout(2)`. setImmediate is the fastest
  // event-loop yield (~0.1 ms vs ~1-2 ms), which matters at the
  // 7.5k-test scale where each saved millisecond compounds. The
  // GET to TRE itself is the floor (~5-15 ms), so we can't go
  // faster than that — but we shouldn't be slower either.
  while (Date.now() < deadline) {
    // Unconfirmed view is the cheap fast path. Some txs (especially
    // late in a long run) only surface via the CONFIRMED view —
    // even with TransactionStore snapshotted, TRE's HTTP layer can
    // index a tx into the solidified-side first. Checking both
    // means we don't sit on a 60 s timeout when the receipt has
    // actually been computed.
    //
    // Use raw HTTP + BigInt-safe JSON parsing — TronWeb's
    // `getUnconfirmedTransactionInfo` quantizes any integer above
    // 2^53 to the nearest IEEE-754 double, which corrupts
    // `callValueInfo[*].callValue` for parseEther-scale transfers
    // (1e18 sun >>> 2^53 ≈ 9e15). chai-matchers' balance assertions
    // then fire off-by-1-wei. Bypassing TronWeb's parser preserves
    // every digit through the JS→Java→JS round-trip.
    const info = await _getInfoBigSafe(tronWeb, txId, '/wallet/gettransactioninfobyid');
    if (info && info.receipt) return info;
    if (iter && iter % 3 === 0) {
      const confirmed = await _getInfoBigSafe(tronWeb, txId, '/walletsolidity/gettransactioninfobyid').catch(() => null);
  ...
  // never reaches solidified state on local TRE.
