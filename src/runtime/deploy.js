...
// receipt-poll fix from wait.js with TronWeb's smart-contract creation
// path.
//
...
// bundled ethers v6 AbiCoder rejects the LITERAL `"tuple"` / `"tuple[]"`
// type strings (it only handles full canonical forms like
// `"(address,uint256)"` / `"(address,uint256)[]"`). The artifact's
// stored ABI uses the literal form (matching solc's JSON output) plus
// a `components` array describing the tuple's fields — we collapse the
// two into one canonical string here.
//
...
// constructor takes a `tuple[]` (e.g. BridgeERC20's
// `(address chain, bytes target)[] remotes`).
function expandTupleTypes(input) {
  ...
  if (m && Array.isArray(input.components)) {
    return { ...input, type: `(${input.components.map(c => expandTupleTypes(c).type).join(',')})${m[1]}` };
  }
  return input;
}
...
// node side — see node_modules/tronweb/lib/esm/lib/TransactionBuilder/
// helper.js:37-43). The returned `signedTx` can be broadcast later via
// `submitPrebuilt` and will deploy to exactly `predictedAddressTron`.
//
// Caveats:
//   - TronWeb defaults `expiration` to `timestamp + 60s`. Broadcasts
//     after that window fail with "Transaction expired". Tests using
//     this should broadcast within the 60s budget.
//   - `ref_block_*` is sampled at prebuild time. java-tron rejects
//     broadcasts whose ref_block is more than 65535 blocks old; under
//     TRE instamine (1 block/broadcast) that's well over a thousand
//     intervening txs of headroom.
async function prebuildDeploy(tronWeb, deployerAddress, artifact, opts = {}) {
  const params = {
    ...
    // Tests that exhaust this (e.g. CREATE2 of full VestingWallet
    // bytecode) surface a real TVM delta and remain unfixable from
    // the spike side.
    ...
    // no effect on bytecode / behavior, so truncating is safe. Hits
    // upstream OZ tests with long mock names like
    // `ERC4626OffsetMockOffsetByOne` (29 chars — borderline) and
    // `$ERC4626DecimalMockExtendedDecimals` (35 chars).
  ...
  // BEFORE sign/broadcast (see node_modules/tronweb/lib/esm/lib/Transaction
  // Builder/helper.js:37-43 — formula `41 + keccak256(txID||owner)[12:]`).
  // Signing does not change raw_data, so signed.txID === unsigned.txID and
  // the predicted address remains correct.
  return { signedTx: signed, predictedAddressTron: signed.contract_address };
}

// Broadcast a pre-signed CreateSmartContract tx and poll for its receipt.
// Returns `{ address, txId, info }`, the same shape as `deployContract`,
// so any consumer that holds a `prebuildDeploy` result can hand it off
// to the rest of the deploy pipeline without branching.
...
//
// NOTE: do NOT call `tre_mine` here on broadcast. Measured behavior on
// tronbox/tre:dev w/ the patched FullNode (oz-spike) shows
...
// just produces an EMPTY block on top — a ~13ms server-side cost per
// deploy that recovers no correctness. Benchmark: 59ms/deploy with
// tre_mine vs 47ms without, across 20 samples. At ~30k deploys per
// full OZ suite that's ~6 minutes of pure waste. The poll below
// catches up to the auto-mined block within 1 iteration in practice.
// (Polling will still mine as a recovery hatch if the tx hasn't
// surfaced — see the `iter % 5` block below.)
async function pollDeployReceipt(tronWeb, txId, opts = {}) {
  // Wall-clock budget for deploy. Default 60s — late in a long test
  // run, TRE's per-block work grows with chain state and deploys can
  // take longer than 30s. 60s gives a wider margin without hiding real
  // hangs.
  //
  // Poll cadence matches waitForReceipt: `setImmediate` yields
  // (~0.1 ms) between RPC fetches instead of `setTimeout(2)`. The
  // GET-getUnconfirmedTransactionInfo round-trip itself is the
  // floor (~5-15 ms), so we just stop adding artificial wait on
  // top of it.
  const deadline = Date.now() + (opts.deployTimeout ?? 180_000);
  let iter = 0;
  // Receipt-result check: TVM sets `contract_address` on the info
  // shape even when the constructor REVERTS — the address is computed
  // deterministically and recorded regardless of execution outcome.
  // Without this gate, deployContract returns a facade for a contract
  // that doesn't actually exist on-chain, and tests like
  ...
  while (Date.now() < deadline) {
    ...
    if (info && info.contract_address) {
      ...
      // Anything else → constructor revert; throw with the same shape
      // as bridge.buildRevertError so chai-matchers can decode.
      if (result === 'SUCCESS' || !result) {
        return { address: info.contract_address, txId, info };
      }
      // Lazy-require the bridge for buildRevertError — deploy.js sits
      // beneath ethers-bridge.js in the module dependency graph and a
  ...
  // Public-network hedge: try the solidified view once before timing out.
