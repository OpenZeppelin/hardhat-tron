...
// `require('./compile')` from `src/index.js`. Side-effects:
//
...
//     source files; see the version-selector module for why.
//   * No `TASK_COMPILE_SOLIDITY_RUN_SOLC` throw. The `GET_SOLC_BUILD`
//     hook unconditionally returns `isSolcJs: true`, so a throw
//     would be unreachable. Dead code surfaces bugs as confusing
//     errors; we'd rather see a real stack trace if invariants
//     break.
//   * No `TASK_CLEAN` wrap unless `separateArtifacts` is enabled.
//     The standard `hardhat clean` already handles `paths.artifacts`
//     / `paths.cache`; only the suffix mode introduces a second set
//     of paths that wouldn't otherwise be cleaned.
