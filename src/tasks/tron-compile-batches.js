//
// src/tasks/tron-compile-batches.js
//
// `tron:compile-batches` task. Compiles a project through tron-solc in
// multiple passes instead of one, so each pass stays under the wasm
// memory ceiling that the 0.8.26 tron-solc build hits on large source
// trees (empirically somewhere between 91 and 226 files for an OZ-v5
// corpus). Artifacts accumulate across passes; the compile cache is
// preserved so shared imports compile only once.
//
...
// Convert a batch definition (dirs + extraLeaves) into the glob array
// consumed by tre.compiler.include. Auto-pairs each `contracts/<x>`
// with `contracts/exposed/<x>` so the wrappers compile in the same
// pass as their originals -- same semantics as the old runtime scan
// in scripts/compile-batches.js, expressed declaratively.
