...
Replaces `@layerzerolabs/hardhat-tron` with:

- **One source of truth for compiler config.** Reads `solidity.compilers` directly — no parallel `tronSolc.compilers` array to keep in sync.
- **SHA-256 verified wasm downloads.** Fetches tron-solc from the canonical `tronprotocol/solc-bin` mirror and verifies each blob against the upstream `list.json` manifest. No more "Tron-solc v0.8.26 is unknown version, attempting anyway..." warnings.
- **Glob include/exclude.** Two `Test.sol` files in different directories are individually addressable; basename-only filtering is gone.
- **No source-file rewriting.** The compiler sees exactly what's on disk; no pragma regex hacks.
- **Stock artifact paths by default.** `artifacts/` and `cache/` — opt into LZ-style `artifacts-tron/` suffix only when you actually need a dual EVM+Tron build.
- **No hardhat-deploy dependency.** Compile and runtime helpers are independent.
...
module.exports = {
  solidity: {
    ...
    settings: {
      ...
      evmVersion: 'cancun',
      viaIR: true,
      // Surface this explicitly -- hardhat-deploy used to inject it
      // silently, removing it would shift the metadata IPFS hash baked
      // into every contract's bytecode.
  ...
  tre: {
    ...
    // Optional: bind-mount a patched FullNode.jar for time-warp + snapshot/revert
    // jarPath: './docker/FullNode.jar',
    compiler: {
      target: 'tron',
...
It also overrides `hre.ethers.{deployContract, getContractFactory, getSigners, getContractAt, provider}` so unmodified ethers-based tests can run against TRE.

## Batched compile

The 0.8.26 tron-solc wasm has a memory ceiling that the OpenZeppelin v5 corpus (~700 files) exceeds in one pass. The `tron:compile-batches` task splits compilation into passes that each stay under the limit. Single-process, shared cache across passes, ~10 min for the full OZ tree.
...
The `docker/` directory contains Java patches that enable time-warp (`time.increase`, `setNextBlockTimestamp`) and snapshot/revert semantics. The patched jar is built locally via:

```bash
npm run docker:build-jar
```

This produces `docker/FullNode.jar` (~200 MB). Reference it from your config via `tre.jarPath: './node_modules/@openzeppelin/hardhat-tron/docker/FullNode.jar'` (or wherever you stage it). Without the patch, the stock `tronbox/tre:dev` image's jar is used and time-warps degrade to real-time waits.
